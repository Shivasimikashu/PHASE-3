...
# Set up Ultrasonic Sensor GPIO pins TRIG_PIN = 17
ECHO_PIN = 18


# Initialize GPIO GPIO.setmode(GPIO.BCM) GPIO.setup(TRIG_PIN, GPIO.OUT)
...
scope = ['https://www.googleapis.com/auth/spreadsheets'] credentials =
ServiceAccountCredentials.from_json_keyfile_name(credentials_file, scope) gc = gspread.authorize(credentials)
worksheet = gc.open(google_sheets_name).worksheet(sheet_name)


try:
while True:
# Measure distance using ultrasonic sensor GPIO.output(TRIG_PIN, False) time.sleep(2)
...
# Determine occupancy status based on distance threshold distance_threshold = 50
...
// JavaScript code to process CSV data and update the website Papa.parse('parkingdata.csv', {
download: true, header: true, skipEmptyLines: true, dynamicTyping: true,
complete: function(results) { const data = results.data;

// Initialize counts let vacantCount = 0;
let occupiedCount = 0;


// Process the data and calculate counts data.forEach((entry) => {
...
document.getElementById('vacantCount').textContent = vacantCount; document.getElementById('occupiedCount').textContent =
occupiedCount;
}
});
